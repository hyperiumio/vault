...
public struct SecureDataMessage {
    ...
    public func decrypt(using messageKey: MessageKey) throws -> Data {
        ...
        return try AES.GCM.open(seal, using: messageKey.value)
    }
...
extension SecureDataMessage {
    ...
    static func encryptContainer(from messages: [Data], using masterKey: MasterKey, cryptor: SecureDataMessageCryptor) throws -> Data {
        let messageKey = MessageKey()
        
        let seals = try messages.map { message in
            try cryptor.encrypt(message, using: messageKey.value)
        }
        
        let tagSegment = seals.map(\.tag).reduce(Data(), +)
        
        let wrappedItemKey = try messageKey.value.withUnsafeBytes { itemKey in
